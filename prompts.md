...
**Goal:** Make the assistant behave like a rigorous fundamentals-first engineering mentor.

**Design choices:** The prompt emphasizes first-principles thinking, Socratic questioning, deliberate practice, and avoiding memorized templates. This fits the assignment requirement that the persona should not be generic; the assistant is instructed to diagnose the missing concept and push the learner toward derivation.

**Few-shot coverage:** The examples cover dynamic programming, system design, and high-volume DSA practice. Each example shows the expected tone: direct, analytical, concise, and ending with a question.

**Key constraints:** No private facts, no real-person impersonation outside the simulation, no insults, and no code unless explicitly requested.
...
**Goal:** Make the assistant connect engineering learning to shipping, product thinking, users, and proof of work.

**Design choices:** The prompt avoids abstract career advice and asks the model to focus on deployment, user feedback, measurable outcomes, and ownership. The persona is warmer than Anshuman but still practical.

**Few-shot coverage:** The examples cover tutorial dependency, DSA versus projects, and local-only apps. These are common student situations where product-minded advice should change behavior.

**Key constraints:** No private facts, no fake career claims, no dishonest portfolio advice, and no code unless explicitly requested.
...
**Goal:** Make the assistant behave like a precise DSA teacher who challenges weak assumptions through edge cases and complexity analysis.

**Design choices:** The prompt emphasizes invariants, constraints, data-structure choice, and proof obligations. Mild dry sarcasm is allowed, but the constraints keep it professional and educational.

**Few-shot coverage:** The examples cover heap recognition, failed greedy logic, and BFS versus DFS. Each response explains how to choose the tool from the problem invariant instead of from pattern memorization.

**Key constraints:** No private facts, no real-person impersonation outside the simulation, mild sarcasm only, and no code unless explicitly requested.
